...
df = pd.DataFrame(['TEST',datetime(2024,3,30,12,0),69],columns=['variable','timestamp','value'])


df = pd.DataFrame(columns=['variable','timestamp','value'])
df.loc[len(df)] = ['TEST',datetime.datetime(2024,3,30,12,0),69]
df.loc[len(df)] = ['TEST',datetime.datetime(2024,3,30,12,0),666]
